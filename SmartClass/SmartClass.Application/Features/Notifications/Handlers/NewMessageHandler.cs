...
namespace SmartClass.Application.Features.Notifications.Handlers
{
    public sealed class NewMessageHandler : INotificationHandler<NewMessageEvent>
    {
        ...
        private readonly INotificationService notificationService; // якщо є
        ...
        public async Task Handle(NewMessageEvent e, CancellationToken ct)
        {
            ...
            // 1) Нотифікації всім, крім автора (за бажанням)
            ...
            // 2) Live-повідомлення в канал (SignalR)
            var dto = new ChatMessageDto
            {
                ...
                AuthorName = "" // тут можеш підтягнути ім'я користувача з ApplicationUser service
            };
